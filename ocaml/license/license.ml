...
let grace_expiry () =
	if Xapi_fist.reduce_grace_period () then
		...
		round_to_days (Unix.time () +. 90. *. 24. *. 60. *. 60.) (* 30 days in the future *)
...
let check_rights l = 
  ...
  let rec mem x l =
    match l with
    [] -> false
    | y::l -> x = (String.uppercase y) || mem x l in
  if l.hwaddr = "00:00:00:00:00:00" then true
  else
    mem l.hwaddr physical_macs
