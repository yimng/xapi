...
let get_appmon ~__context ~self = 
	let domid = Int64.to_int (Db.VM.get_domid ~__context ~self) in
	let xs = Xs.daemon_open () in
	let path = xs.Xs.getdomainpath domid ^ "/data/appmon" in
	try	
	 xs.Xs.read path
	with
	_ -> "<error>"
(*
	...
	Vmopshelpers.with_xs (fun xs -> xs.Xs.read (xs.Xs.getdomainpath domid ^ "/console/tty"))
*)
