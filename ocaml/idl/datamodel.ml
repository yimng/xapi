...
let session_login_with_cert  = call ~flags:[]
  ...
  ~result:(Ref _session,"reference of newly created session")
  ~versioned_params:
   [{param_type=String; param_name="clientip"; param_doc="client ip"; param_release=rio_release; param_default=None};
	{param_type=String; param_name="appid"; param_doc="app id"; param_release=rio_release; param_default=None};
...
let vm_get_appmon = call
       ~name:"get_appmon"
       ~in_product_since:rel_orlando
       ~doc:"Returns the appmon num which in datasource"
       ~params:[Ref _vm, "self", "The VM"]
       ~flags:[`Session]
       ~result:(String, "The result of appmon")
       ~allowed_roles:_R_VM_ADMIN
       ()
...
let host_eject = call
  ~name:"eject"
  ~in_product_since:rel_rio
  ~doc:"Eject CD/DVD Rom From Host. (if there is a CD/DVD Device)"
  ~params:[Ref _host, "host", "The Host CD/DVD Eject"]
  ~allowed_roles:_R_POOL_OP
  ()
...
let host =
    create_obj ~in_db:true ~in_product_since:rel_rio ~in_oss_since:oss_since_303 ~internal_deprecated_since:None ~persist:PersistEverything ~gen_constructor_destructor:false ~name:_host ~descr:"A physical host" ~gen_events:true
      ~doccomments:[]
      ~messages_default_allowed_roles:_R_POOL_OP
      ~messages: [host_disable; host_enable; host_shutdown; host_eject; host_reboot; host_dmesg; host_dmesg_clear; host_get_log; host_send_debug_keys; host_bugreport_upload; host_list_methods; host_license_apply; host_create; host_destroy; 
		  host_power_on;
