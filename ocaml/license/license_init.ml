...
let initialise ~__context ~host =
	try
		...
		debug "initialise : existing license %s" edition;
		let params =
			match edition with
			"free" -> ["force", "true"] @ (License.to_assoc_list (License.default ()))
			|x -> ["force", "true"] @ (Db.Host.get_license_params ~__context ~self:host)
		in

		let edition', features, additional =
			v6_initialise ~__context ~edition ~params:params in
