...
module Forward = functor(Local: Custom_actions.CUSTOM_ACTIONS) -> struct
	...
	module VM = struct
      ...
      let get_appmon ~__context ~self =
      info "VM.get_appmon: VM = '%s'" (vm_uuid ~__context self);
      let local_fn = Local.VM.get_appmon ~self in
         forward_vm_op ~local_fn ~__context ~vm:self
          (fun session_id rpc -> Client.VM.get_appmon rpc session_id self)
	...
	module Host = struct
		...
		let eject ~__context ~host =
			info "Host.eject: host = '%s'" (host_uuid ~__context host);
			let local_fn = Local.Host.eject ~host in
			with_host_operation ~__context ~self:host ~doc:"Host.eject" ~op:`eject
				(fun () ->
					do_op_on ~local_fn ~__context ~host (fun session_id rpc -> Client.Host.eject rpc session_id host)
				)
